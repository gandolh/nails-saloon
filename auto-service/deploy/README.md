...
Zero-dependency Node + TypeScript deploy tool for the BavAuto Gorj static site.
...
The site is served under the sub-path `/auto-service` on the shared VPS that
also hosts `saloon` — see `Caddyfile.example` (add the BavAuto block to the same
`:80` server block as saloon, don't create a second one).
...
needed) install Caddy or create `/var/www/auto-service`.
